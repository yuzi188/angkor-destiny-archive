...
function buildPrompt(payload: ClaimRequest) {
  ...
  return [
    "你是吳哥午夜檔案 Angkor Destiny Archive 的命運檔案撰寫者。",
    ...
    "禁用詞：八字、星盤、星座、命盤、五行、流年、大運、十神、命理、三叉分析。請改用吳哥午夜檔案語言。",
...
function buildTransferSectionPrompt(payload: ClaimRequest, section: ReportSection, index: number) {
  ...
  return [
    ...
    "禁用詞：八字、星盤、星座、命盤、五行、流年、大運、十神、命理、三叉分析。請全部轉成吳哥午夜檔案世界觀語言。",
...
function makeFilename(payload: ClaimRequest) {
  ...
  const plan = getPlan(payload.productId);
  return `吳哥午夜檔案_${plan.name}_${passengerName}.txt`;
}
...
async function sendIntroWithResend(to: string, subject: string, payload: ClaimRequest) {
  ...
  const plan = getPlan(payload.productId);
  const passenger = payload.passenger ?? {};
  const from = process.env.REPORT_FROM_EMAIL || "吳哥午夜檔案 <onboarding@resend.dev>";
  const text = [
    "你的吳哥午夜檔案班次已經受理。",
    "",
    `方案：${plan.name}`,
...
async function legacyPOST(request: Request) {
  try {
    ...
    if (shouldSendIntroFirst(payload.productId)) {
      const introSubject = `吳哥午夜檔案｜你的班次已受理｜${introPassengerName}`;
      const fullSubject = `吳哥午夜檔案｜${plan.name}｜${introPassengerName}`;
    ...
    const subject = `吳哥午夜檔案｜${plan.name}｜${passengerName}`;
...
async function queuedMemoryPOST(request: Request) {
  try {
    ...
    if (shouldSendIntroFirst(payload.productId)) {
      ...
      const introSubject = `吳哥午夜檔案｜你的班次已受理｜${passengerName}`;
      const fullSubject = `吳哥午夜檔案｜${plan.name}｜${passengerName}`;
      const introResult = await sendIntroWithResend(recipient, introSubject, payload);
...
export async function POST(request: Request) {
  try {
    ...
    if (shouldSendIntroFirst(payload.productId)) {
      ...
      const introResult = await sendIntroWithResend(recipient, `吳哥午夜檔案｜你的班次已受理｜${passengerName}`, payload);
      if (!introResult.sent) {
